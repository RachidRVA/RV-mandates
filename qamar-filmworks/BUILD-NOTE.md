...
| **Lune du héros** | Image PNG détourée (fond transparent) posée à droite du bloc héros | Dans Site Creator : ajouter un **bloc image** dans le héros, aligné à droite, et **téléverser la lune définitive — NASA PIA00405** (domaine public, aucune attribution requise ; fichier `PIA00405~large.jpg`, à passer en niveaux de gris pour rester sobre). *La maquette montre une lune provisoire (extraite de l'ancien visuel) car cet environnement ne peut pas récupérer l'image NASA en ligne.* Aucun halo, aucun reflet. |
